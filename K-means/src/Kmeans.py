...
# df_clustered = k_means(df, 10)
# df_clustered.to_csv('K-means\CSV\hepatitis_clustered.csv', index=False)
def clust(path, k):
    df = read_data(path)
    df_clustered = k_means(df, k)
    # df_clustered.to_csv('K-means\CSV\clustered.csv', index=False)
    return df_clustered